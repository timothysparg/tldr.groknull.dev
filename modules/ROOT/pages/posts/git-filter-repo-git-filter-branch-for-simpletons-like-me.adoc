...
:icons: font
:page-layout: article
:article-card-image-light: image$git-filter-for-simpletons-light.png
:article-card-image-dark: image$git-filter-for-simpletons-dark.png

Also if you're not a simpleton it has the benefit of being significantly faster, but I digress. 

[TLDR]
====
__git filter-repo --replace-text <(echo "filter-branch==>filter-repo")__
====
